...
Common modes:
=============
 i. TCP connect scan (-sT)
    Completes the full TCP 3‑way handshake.
    Pros: Works as non‑root, very reliable.
    Cons: Noisy in logs (every connection is real).
 Example:
    nmap -sT 192.168.1.100 -sT

Sends SYN; if SYN/ACK, immediately sends RST, never finishes handshake.

Pros: Faster and stealthier than -sT; standard for pentests.

Cons: Needs root; still visible to IDS/IPS.

Used in your lab together with version detection:
...
Critical for mapping to CVEs and CVSS scores.

Example:

bash
nmap -sV 192.168.1.100
In theory answers, you describe network scanning as: identify live hosts, enumerate ports, fingerprint services/OS, and produce input for vuln scanning.
​
